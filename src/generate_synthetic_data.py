            ...
            rows.append(
                {
                    ...
                    "clicked": clicked,
                    "reported": reported,
                    "creds_submitted": creds,
                    "origin_type": "employee",
                }
            )
    pd.DataFrame(rows).sort_values("timestamp").to_csv(
        DATA_DIR / "phishing_simulation.csv", index=False
    )
        ...
        for ts in _random_times(random.randint(8, 25)):
            ...
            data_gb = round(
                (
                    ...
                    if action in ("download", "write", "modify")
...
def generate_policy_violations(users_df):
    kinds = [
        "Access Violation",
        "Data Transfer Without Approval",
        "Policy Breach",
        "Phishing Response",
    ]
    rows = []
            ...
            rows.append(
                {
                    "user_id": u["user_id"],
                    "timestamp": ts,
                    "violation_type": random.choice(kinds),
    ...
    pd.DataFrame(rows).sort_values("timestamp").to_csv(
        DATA_DIR / "policy_violations.csv", index=False
    )
