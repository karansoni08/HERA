...
def build_user_features():
    ...
    if (DATA_DIR / "phishing_simulation.csv").exists():
        ...
        ph_agg = (
            ph.groupby("user_id")
            .agg(
                attempts=("opened", "count"),
                opens=("opened", "sum"),
                clicks=("clicked", "sum"),
                reports=("reported", "sum"),
                creds_submitted=("creds_submitted", "sum"),
            )
            .reset_index()
        )
        ph_agg["click_rate"] = ph_agg.apply(lambda r: safe_div(r["clicks"], r["attempts"]), axis=1)
        ph_agg["report_rate"] = ph_agg.apply(
            lambda r: safe_div(r["reports"], r["attempts"]), axis=1
        )
    ...
    if (DATA_DIR / "access_logs.csv").exists():
        ...
        al["download_gb"] = np.where(
            al["action"].isin(["download", "write", "modify"]), al["data_gb"], 0.0
        )
        ...
        al_agg = (
            ...
            .agg(
                events=("action", "count"),
                downloads_gb=("download_gb", "sum"),
                after_hours_events=("after_hours", "sum"),
                anom_geo_events=("anomalous_geo", "sum"),
                escalate_events=("escalate_flag", "sum"),
                delete_events=("delete_flag", "sum"),
            )
            .reset_index()
        )
        al_agg["after_hours_rate"] = al_agg.apply(
            lambda r: safe_div(r["after_hours_events"], r["events"]), axis=1
        )
        al_agg["anom_geo_rate"] = al_agg.apply(
            lambda r: safe_div(r["anom_geo_events"], r["events"]), axis=1
        )
    else:
    ...
    if (DATA_DIR / "policy_violations.csv").exists():
        ...
        pv_agg = (
            ...
            .agg(
                ...
                viol_repeat_sum=("repeat_count", "sum"),
            )
            .reset_index()
        )
    ...
    df = (
        users.merge(ph_agg, on="user_id", how="left")
        .merge(al_agg, on="user_id", how="left")
        .merge(pv_agg, on="user_id", how="left")
    ).fillna(0)
