...
def load_scores():
    ...
    for c in ["user_id", "department", "role", "HTI", "risk_band"]:
        if c not in df.columns:
            raise RuntimeError(f"Missing {c} in risk_scores.csv")
    return df
...
def chart_avg_hti_by_department(df):
    ...
    ax.set_title("Average HTI by Department")
    ax.set_ylabel("HTI (0–100)")
...
def chart_top_risky_users(df, n=15):
    ...
    ax.set_title(f"Top {n} Risky Users (HTI)")
    ax.set_ylabel("HTI (0–100)")
...
def chart_hti_hist_by_role(df):
    ...
    plt.title("HTI Distribution by Role (KDE)")
    plt.xlabel("HTI")
    plt.legend()
...
def chart_risk_band_counts(df):
    ...
    ax.set_title("Users by Risk Band")
    ax.set_ylabel("Count")
